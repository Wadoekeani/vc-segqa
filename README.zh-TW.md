...
限制，依重要性排：十組配對切面、每半五組，對一個 141 mm 長的物件來說很稀疏。二維切面、
一條縱剖面。分帶依賴擬合出的軸心：把軸心移 60 voxel（144 µm），配對的 coherence 差動
0.008、coverage 差動 2 個百分點（四個方向 × 十張切面的中位數），對照要量的效應是 0 與
15 個百分點。上半部的 coverage 差在 40／60／80 百分位門檻下是 −17／−13.5／−7 個百分點
——方向一致為負，大小隨門檻變。已追蹤帶是 w010 半徑到 1.8 倍、以視窗邊緣為上限；有兩個
高度 w010 已經到了視窗邊緣，那兩個沒有對照，只計入核心那一側的數字。而「slice 大＝頂部」
是 FAQ 對 2023 體積的陳述，帶到 2026 體積只靠 `transform.json`（它的 `fixed_volume` 就
是那個 2023 體積，z→z 係數為正）。
